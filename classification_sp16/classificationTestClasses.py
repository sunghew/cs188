...
class UpdatesEqualityTest(testClasses.TestCase):
    ...
    def update_and_get_param_values(self, moduleDict):
        # dataset
        ...
        old_param_values = model.get_param_values()

        tfu.get_session().run(update_ops, feed_dict=feed_dict)
        new_param_values = model.get_param_values()
        return old_param_values, new_param_values, grads

    def execute(self, grades, moduleDict, solutionDict):
        old_param_values, new_param_values, grads = self.update_and_get_param_values(moduleDict)
        ...
        correct = all([np.allclose(new_param_value, gold_param_value, atol=1e-07) for (new_param_value, gold_param_value) in zip(new_param_values, gold_param_values)])
        if correct:
        ...
        self.addMessage('Parameter values after updates (with %s) are not equal.\n' % hyperparameters_str)
        self.addMessage('Parameter values before updates were:\n%r\n' % old_param_values)
        self.addMessage('Gradients with respect to each parameter were:\n%r\n' % grads)
        self.addMessage('Student\'s parameter values after updates are:\n%r\n' % new_param_values)
        self.addMessage('Correct parameter values after updates are:\n%r\n' % gold_param_values)
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        np.savez(fname, *new_param_values)
