...
class PerceptronClassifier(object):
    ...
    def train(self, input_train_data, label_train_data, input_val_data, label_val_data, iterations, callback=None):
        ...
        for iteration in range(iterations):
            ...
            for i in range(len(input_train_data)):
                ...
                if classified[0] != label_train_data[i]:
                    self.weights[label_train_data[i]] += input_train_data[i]
                    self.weights[classified[0]] -= input_train_data[i]
    ...
    def find_high_weight_features(self, label):
        ...
        "*** YOUR CODE HERE ***"
        import operator
        weights = sorted(self.weights[label].items(), key=operator.itemgetter(1))

        for i in range(-1, -101, -1):
            best100Features.append(weights[i][0])
